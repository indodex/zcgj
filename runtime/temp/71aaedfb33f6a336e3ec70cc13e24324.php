    ...
    <div class="main-content-inner">
      <div class="breadcrumbs" id="breadcrumbs">
        <ul class="breadcrumb">
          ...
          <li> <a href="<?php echo url('page'); ?>">单页信息列表</a> </li>
          ...
          <div class="col-xs-12"> 
            ...
            <div class="row">
              <div class="col-xs-12" style="margin-bottom:10px;">
                <form action="<?php echo url('page'); ?>" method="get" class="form-inline" role="form">
                  <div class="form-group">
                    <label>关键词：</label>
                    <input name="keywords" type="text" class="form-control search" placeholder="单页名称">
                  </div>&nbsp;&nbsp;
                  <button type="submit" class="btn btn-sm btn-primary">查询</button>
                  <a class="btn btn-sm btn-success" style="float:right; margin-right:10px;" href="<?php echo url('add_page'); ?>" >添加单页</a>
              ...
              <div class="col-xs-12">
                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                  <thead>
                    <tr>
                      <th class="center" width='5%'>单页ID</th>
                      <th width='10%'>单页名称</th>
                      <th width='55%'>内容</th>
                      <th width='5%'>创建时间</th>
                      <th width='5%'>更新时间</th>
                  ...
                  <tbody class='t_tbody'>
                    <?php if(is_array($list['list']) || $list['list'] instanceof \think\Collection || $list['list'] instanceof \think\Paginator): $k = 0; $__LIST__ = $list['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?>
                      <tr>
                        ...
                        <td>
                          <a class="btn btn-sm btn-success" href="<?php echo url('edit_page',array('id'=>$vo['id'])); ?>" >修改</a>
                          <!--<a class="btn btn-sm btn-danger" href="javascript:void(0);" onclick="deletePage(this,<?php echo $vo['id']; ?>)">删除</a>-->
                        </td>
                      </tr>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
                  </tbody>
...
<script type="text/javascript">
  $('a[href="/Admin/Menu/page"]').parents().filter('li').addClass('open active');
...
jQuery(function($) {
  ...
  $(document).on('click', 'button:reset',function() {
    location.href = '<?php echo url('page'); ?>';
  }); 
});

// 删除单页
function deletePage(obj,id){
	layer.confirm('确定要删除吗？<br>该单页所有的信息都将被完全删除，不可恢复！', {
		btn: ['确定','关闭'] //按钮
	}, function(){
		$.post("<?php echo url('delete_page'); ?>", {id: id}).success(function(data) {
			if (data.code == 0) {
