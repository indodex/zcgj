                  ...
                  <div class="form-group"><label>状态</label>
                    <select name="status" class="form-control" <!--onchange='look_state(this)'-->>
                      ...
                      <?php if(is_array($state) || $state instanceof \think\Collection || $state instanceof \think\Paginator): $i = 0; $__LIST__ = $state;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                        <option value="<?php echo $vo['value']; ?>" <?php if($get_state == $vo['value']): ?>selected='selected'<?php endif; ?>><?php echo $vo['key']; ?></option>
                      <?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                  </div>&nbsp;&nbsp;
                  ...
                  <tbody>
                    <?php if(is_array($list['list']) || $list['list'] instanceof \think\Collection || $list['list'] instanceof \think\Paginator): $k = 0; $__LIST__ = $list['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?>
                      <tr>
                        ...
                        <td><?php if($vo['parent_account'] == ''): ?>--<?php else: ?><?php echo $vo['parent_account']; endif; ?></td>
                        <td><?php echo $vo['shop_text']; ?></td>
                        <td>
                        	<?php if($vo['id'] != '1'): ?>
                        		<div class='<?php echo $vo['button']; ?>' onclick="change_status(<?php echo $vo['id']; ?>,<?php echo $vo['status']; ?>)"><?php echo $vo['state_type']; ?><div>
                        	<?php else: ?>
                        		-
                        	<?php endif; ?>
                        </td>
                        <td class='user_info_btn'>
                        	<a class="btn btn-sm btn-success" href="<?php echo url('detail',array('id'=>$vo['id'])); ?>" >详情</a>
                        	<a class="btn btn-sm btn-success" href="<?php echo url('finance',array('id'=>$vo['id'])); ?>" >资产</a>
                        	<?php if($vo['shop_apply'] != '0'): ?>
                        		<a class="btn btn-sm <?php echo $vo['apply_color']; ?>" href="<?php echo url('shop',array('id'=>$vo['id'])); ?>" ><?php echo $vo['shop_apply_text']; ?></a>
                        	<?php endif; ?>
                        </td>
                        <td>
                        	<?php if($vo['id'] != '1'): ?>
...
function change_status(id,state){
	layer.confirm('确定要修改吗？',{
		...
		$.post('<?php echo url("state"); ?>',{id:id,state:state}).success(function(data){
			if(data.code == 0){
...
function deleteInfo(obj,id){
	layer.confirm('确定要删除吗？<br>该用户所有的信息都将被完全删除，不可恢复！', {
		btn: ['确定','关闭'] //按钮
	}, function(){
