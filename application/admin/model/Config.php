...
class Config extends Base
{
    ...
    {
        $list = $this->where(array('url' => $url,'state' => 1))->order('id asc')->select();
        return $list;
    }
    ...
    {
    	if($datas['START_ID']){
    		$result = Db::query('show table status like "sn_user"');
    		$last_id = $result[0]['Auto_increment'];
    		if($datas['START_ID'] < $last_id){
    			return ['status' => 0,'info' => '设置的ID不能小于'.$last_id];
    		}
    		Db::query('alter table sn_user AUTO_INCREMENT='.$datas['START_ID']);
    	}
